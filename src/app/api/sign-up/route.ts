...
export async function POST(request: Request) {
    ...
    try {
        ...
        const existngUserByEmail = await UserModel.findOne({ email })
        ...
        if (existngUserByEmail) {
            if (existngUserByEmail.isVerified) {
                ...
                existngUserByEmail.password = hashedPassword;
                existngUserByEmail.verifyCode = verifyCode;
                existngUserByEmail.verifyCodeExpiry = new Date(Date.now() + 3600000);
                await existngUserByEmail.save();
            }
        }
        else {
            const hashedPassword = await bcrypt.hash(password, 10)
